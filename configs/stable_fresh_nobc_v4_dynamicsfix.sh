...
SESSION_NAME="crl_fresh_nobc_v4_$(date +%Y%m%d_%H%M%S)"
...
CMD=(
  ...
  # Fresh run, no BC/checkpoint resume. Keep successful scripted data in replay as warmstart.
  ...
  # More aggressive exploration with a non-trivial floor.
  --warmup-episodes 120
  ...
  # Keep entropy from collapsing.
  --target-entropy-scale -0.25
  --min-log-alpha -4.0
  --max-log-alpha 2.0

  # Paper-aligned contrastive update: future achieved goals are positives; no reward/BC actor shortcuts.
  --energy-fn dot
  ...
  # Increase control authority so exploration can move farther.
